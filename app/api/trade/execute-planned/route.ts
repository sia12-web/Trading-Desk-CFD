...
export async function POST(req: Request) {
    ...
    const instrument = isCrypto
        ? `CRYPTO_${trade.pair.replace('/', '_').replace('POL', 'MATIC')}` // Kraken POL is still MATIC in our system or vice versa? 
                                                                         // Actually, the instrument in kraken client map should be handled.
        : trade.pair.replace('/', '_')
    
    // In our system we use CRYPTO_BTC_USD
    const internalInstrument = isCrypto ? `CRYPTO_${trade.pair.replace('/', '_')}` : instrument
    ...
    if (!riskResult.passed) {
        await logExecution({
            ...
            action: 'place_order',
            trade_id: tradeId,
            request_payload: { tradeId, instrument: internalInstrument },
    ...
    try {
        ...
        if (isCrypto) {
            ...
            brokerResponseData = { broker: 'kraken', order_id: brokerTradeId }
